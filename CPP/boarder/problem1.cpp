...
bool IdentifierValidity(char* str)
{
    ...
    while (right <= len&& left <= right)
        {
            ...
            else if (isDelimiter(str[right]) == true &&left != right || (right == len&& left != right))
            {
                    ...
                    else if (IdentifierValidity(subStr) == true &&isDelimiter(str[right - 1]) == false)
                        printf("'%s' IS A VALID IDENTIFIER\n", subStr);
                    else if (IdentifierValidity(subStr) == false &&isDelimiter(str[right - 1]) == false)
